    ...
    <div class="row cuerpo">
      ...
      <div class="col s10 offset-s1 tabla">
        <table class="centered striped resp2">
          ...
          <tbody id="filas">
            @forelse($tiposterritorio as $tipo)
                <tr id="fila{{$tipo->id}}">
                  <td id="filanombre{{$tipo->id}}">{{$tipo->nombre}}</td>
                  <td id="filaeditar{{$tipo->id}}"><a onclick="modaleditartipoterritorio({{$tipo->id}},'{{$tipo->nombre}}','{{$tipo->abreviatura}}')" class="btn green"><i class="material-icons">edit</i></a></td>
                  <td><a onclick="modaleliminar('tipoterritorio',{{$tipo->id}})" class="btn red"><i class="material-icons">delete</i></a></td>
                </tr>
            @empty
                <tr id="filaempty">
                    <td colspan="3">No hay tipos de territorio</td>
                </tr>
            @endforelse
          </tbody>
        </table>
        <div class="row center">
            {{$tiposterritorio->links()}}
        </div>
      </div>
    </div>
  ...
  <div class="row">
    ...
    <div class="row center botones">
        <div class="col s6 center" id="divbtnagregar">
            <button onclick="tipoterritoriopost('agregar')" class="btn-large">GRABAR</button>
        </div>
  ...
  <div class="row">
    ...
    <div class="row center botones">
        <div class="col s6 center" id="divbtneditar">
            <button onclick="tipoterritoriopost('editar')" class="btn-large">GRABAR</button>
        </div>
        <div class="col s6 center">
            <button class="modal-action modal-close btn-large red waves-effect">CERRAR</button>
        </div>
