@include('include.head')
    @include('include.menu-usuario')
    @include('include.modal-logout')
    
    <!--Cuerpo-->
      <div class="content-wrapper">
          <div class="container-fluid">
              <div class="card mb-3">
                  <div class="card-header text-size-ssm"><i class="icon-user-tie mr-2 text-size-ssm"></i>Usuarios</div>
                  <div class="card-body">

                <!-- Modal TipoUsuario -->
                <div class="modal fade bd-example-modal-sm" id="modalUniversidades" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-lg" role="document">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title w-100">Registro de Permisos</h5>
                          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                          </button>
                        </div>
                        <div class="modal-body mx-2">
                            <div class="text-center">
                                <div class="icon-object border-primary-300 text-primary-300 mb-4"><i class="icon-reading"></i></div>
                            </div>
                            <!-- Formulario TipoUsuario-->
                            <form id="formMantenimiento" action="mantenimiento/permisos" onsubmit="validar();return false;">
                                {{ csrf_field() }}
                                <input type="hidden" name="modo" id="inptModo">
                                <input type="hidden" name="id_tipo" value="{{$tipo->id}}">
                                <input type="hidden" name="id" id="inptId">
                            </form>
                            <!-- End Formulario TipoUsuario-->    
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
                            <button type="button" class="btn btn-success" onclick="validar();" id="btnRegistrar">Registrar</button>
                        </div>
                        </div>
                    </div>
                </div>
                <!-- End Modal TipoUsuario-->
                    <div>
                        PERMISOS PARA {{$tipo->nombre}}
                    </div>
            <!--End Search-->
            <div class="row mb-3">
                <div class="col">
                    <div>ACTUALES</div>
                    <div class="table-responsive-lg">
                        <table class="table text-size-xm">
                            <thead class="thead-light">
                                <tr>
                                    <th scope="col">N°</th>
                                    <th scope="col">Nombre</th>
                                    <th scope="col"></th>
                                </tr>
                            </thead>
                            <tbody class="lead text-size-xm">
                                @php($w=0)
                                @foreach($menus as $menu)
                                @if($menu->id_permiso>0)
                                @php($w=$w+1)
                                <tr>
                                    <td>{{$w}}</td>
                                    <td>{{$menu->nombre}}</td>
                                    <td><img class="icon-t" src="img/iconos/borrar.svg" alt="Eliminar" onclick="eliminar({{$menu->id_permiso}});"></td>
                                </tr>
                                @endif
                                @endforeach
                            </tbody>
                        </table>
                    </div>        
                </div>
                <div class="col">
                    <div>RESTANTES</div>
                    <div class="table-responsive-lg">
                        <table class="table text-size-xm">
                            <thead class="thead-light">
                                <tr>
                                    <th scope="col">N°</th>
                                    <th scope="col">Nombre</th>
                                    <th scope="col"></th>
                                </tr>
                            </thead>
                            <tbody class="lead text-size-xm">
                                @php($w=0)
                                @foreach($menus as $menu)
                                @if($menu->id_permiso<=0)
                                @php($w=$w+1)
                                <tr>
                                    <td>{{$w}}</td>
                                    <td>{{$menu->nombre}}</td>
                                    <td><img class="icon-t" src="img/iconos/editar.svg" alt="Agregar" onclick="agregar({{$menu->id}});"></td>
                                </tr>
                                @endif
                                @endforeach
                            </tbody>
                        </table>
                    </div>        
                </div>
            </div>
            
            </div>
            </div>
              </div>
          </div>
      </div>
      <!--End Cuerpo-->

    @include('include.footer')
<script>
    
    function agregar(id){
        //enviarURL($("#formMantenimiento").prop("action")+'?modo=A','&id='+id,"buscar();");
        $("#inptId").val(id);
        $("#inptModo").val("N");
        AlertaForm("formMantenimiento","¿DESEAS AREGAR EL REGISTRO?","","enviarForm('formMantenimiento','xyz');");
    }
    
    function eliminar(id){
        //enviarURL($("#formMantenimiento").prop("action")+'?modo=A','&id='+id,"buscar();");
        $("#inptId").val(id);
        $("#inptModo").val("A");
        AlertaForm("formMantenimiento","¿DESEAS ELIMINAR EL REGISTRO?","","enviarForm('formMantenimiento','xyz');");
    }
    
</script>