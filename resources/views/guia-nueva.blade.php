    ...
    <div class="row cuerpo">
      ...
      <div class="row">
            <div class="col s6 offset-s3 center">
                <form id="formguia" method="POST">
                    {{ csrf_field() }}
                    <input type="hidden" name="id" id="id-contrato" value="{{$contrato->id}}">
      ...
      <div class="col s10 offset-s1">
          <div class="row">
              <form id="detalleguia" action="detalleguia" method="POST">
                  {{ csrf_field() }}
                  <div class="col s12 m12 l8 input-field">
                      <select name="modelo" id="modelos" style="width: 100%;" onchange="elegirproductodetalleguia()">
                      <option value="0">ELIJA UN PRODUCTO</option>
                      @forelse($modelos as $modelo)
                          <option value="{{$modelo->id}}" id="modelo{{$modelo->id}}" data-href="{{$modelo->cantidad-$modelo->registrados}}">{{$modelo->nombretipo}} {{$modelo->nombremarca}} {{$modelo->nombremodelo}}</option>
                      @empty
                ...
                <div class="col s12 m6 l2 input-field">
                    <input type="number" id="restantes" readonly>
                    <label for="restantes" id="lbl-restantes">Cantidad Restante</label>
                </div>
                <div class="col s12 m6 l2 input-field">
                    <input type="number" id="cantidad" min="1" step="1">
                    <label for="cantidad" id="lbl-cantidad">Cantidad a ingresar</label>
                </div>
              </form>
          </div>
      </div>
                ...
                <div class="col s10 offset-s1">
                    <form id="guiaexcelform">
                      ...
                      <div class="col s12 m6 l1 offset-s1 input-field" id="divbtnagregar1">
                          <a onclick="agregardetalleguia(1)" class="btn red"><i class="material-icons">add</i></a>
                      </div>
                    </form>
              </div>
            </div>
            ...
            <div class="row">
                <div class="col s10 offset-s1">
                      ...
                      <div class="col s12 m6 l1 offset-s1 input-field" id="divbtnagregar2">
                          <a onclick="agregardetalleguia(2)" class="btn red"><i class="material-icons">add</i></a>
                      </div>
              </div>
            </div>
        </div>
      ...
      <div class="col s12 center input-field" id="divbtnregistrar">
            <a onclick="registrarguia()" class="btn-large">REGISTRAR<i class="material-icons right">save</i></a>
        </div>
    </div>
